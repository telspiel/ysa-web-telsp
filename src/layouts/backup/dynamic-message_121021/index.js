...
$("#addToMsg").click(function (e) {
  ...
  const contentTemplateId1 = $("#templatename").children("option:selected").val().trim();
  if (!contentTemplateId1) {
    if ($("#columnList").val() !== "null") {
      $("#msgText").val(textBefore + "" + $("#columnList").val() + "" + textAfter);
    } else {
      Alert.clearAll();
  ...
  Request(
    ...
    if (Endpoints.validateResponse(resData)) {
      ...
      Object.keys(tData).forEach(function (key) {
        ...
        html = html + "<tr><td>" + key + "</td><td class='text-wrap'>" + tData[key] + "</td></tr>";
        console.log(html);
      });
      $("#tBody").html(html);
...
$("#templatename").change(function () {
  ...
  if (txt) {
    $("#msgText").val(txt.templateText).prop("readonly", true);
