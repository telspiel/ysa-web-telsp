...
// if small id="shortURLNote does not exist then remove i tag

// function removeShortUrlNote() {
//   if (!document.getElementById("shortURLNote")) {
//     document.getElementById("shortURLNote").remove();
//   }
// }

// const shortURLNoteid = document.getElementById("shortURLNote");

// $(document).ready(function () {
//   if (shortURLNoteid==null) {
//     console.log("shortURLNote not exist");
//     ('#disnote').remove();
//   }
// });
  ...
  Request(Endpoints.get("uploadFile"), "POST", formData, {
    ...
    if (Endpoints.validateResponse(data)) {
      if (data.result === "Success") {
        ...
        fileName = "test101-1641875405541TestingXls-File5.txt";
      }
      ...
      const totalCount = parseInt(data.totalNumber);
      if(totalCount !== null){
        $("#totalMobile").html(totalCount);
      }
      
      uploadedFilesList = data.data.uploadedBulkfileName;
...
$("#bulk-upload").submit(function (e) {
  ...
  if (splitFile == "yes") {
    ...
    var scheduleDateTime = $("#scheduleDate").val();
    if (!scheduleDateTime) {
  ...
  if (splitFile == "yes") {
    ...
    $.each(obj, function (key, value) {
      ...
      if (value.from === '' || value.to === '' || value.hh === '' || value.mm === '') {
        Alert.clearAll();
        Alert.error("Please Fille the Split file Field", { clearTime: 10 * 1000 });
      ...
      if (parseInt(value.from) > parseInt(value.to) || parseInt(value.to) > totalNumbers) {
        dateValidation = false;
        Alert.error("Split File Data Mismatch", { clearTime: 10 * 1000 });
        return;
      }
    });
...
function sendBulkSMS() {
  ...
  data1["uploadedBulkfileName"] = uploadedFilesList;
